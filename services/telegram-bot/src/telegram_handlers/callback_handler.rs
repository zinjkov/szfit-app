...
fn handlers() -> HashMap<Screen, Box<dyn CallbackHandler>> {
    let mut handlers: HashMap<Screen, Box<dyn CallbackHandler>> = HashMap::new();
    handlers.insert(Screen::WorkoutList, Box::new(WorkoutListHandler{}));
    handlers.insert(Screen::ExerciseList, Box::new(ExerciseList{}));
    handlers.insert(Screen::WorkoutInProgress, Box::new(WorkoutInProgressHandler{}));
    handlers.insert(Screen::ExerciseInProgress, Box::new(ExerciseInProgress{}));
    handlers
}

impl CallbackHandlerProcessor {
    pub fn new(bot: Bot, callback_query: CallbackQuery, user_dialog: UserDialogue, catalog: Catalog) -> Self {
        Self { bot, callback_query, user_dialog, catalog, screen_handlers: handlers()}
    }
    pub async fn process(self) -> HandlerResult<()> {
        let screen = self.current_screen().await?;
        log::error!("Current screen: {:?}", screen);
        let _ = self.screen_handlers
            .get(&screen)
            .ok_or(HandlerError::CallbackNotFound)
            .inspect_err(|err| log::error!("callback not found: {:?}", err))?
            .apply(&self)
            .await?
