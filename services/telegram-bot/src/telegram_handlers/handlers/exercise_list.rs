    ...
    async fn apply(
        ...
        match data.callback_type {
            WorkoutPlanCallbackType::Start => {
                let workout_id: Id =
                    data.workout_id.ok_or(HandlerError::WrongArgs)?.into();
                let mut user_state =
                    chp.user_dialog.get_or_default().await?;
                ...
                let exercise_list = exercise_list
                    ...
                    .collect();
                user_state.workout_usecase =
                    Some(Arc::new(Mutex::new(usecase)));
                chp.user_dialog.update(user_state).await?;

                reply = workout_progress_view(exercise_list).into_reply();

                set_state(&chp.user_dialog, Screen::WorkoutInProgress)
                    .await?;
            }
