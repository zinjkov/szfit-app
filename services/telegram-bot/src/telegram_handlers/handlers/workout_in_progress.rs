...
impl CallbackHandler for WorkoutInProgressHandler {
    async fn apply(
        ...
        usecase.lock().await.start_exercise(exercise_id);

        let exercise_repo =
            chp.catalog
                .get_one::<dyn repositories::IExerciseRepository>()?;
        let exercise_name =
            exercise_repo.find_by_id(exercise_id).await?.name;
        let max_last_set = usecase.lock().await.last_max_set().await;
        let reply = exercise_progress_view(exercise_name, max_last_set)
            .into_reply();

        let _ =
            set_state(&chp.user_dialog, Screen::ExerciseInProgress).await;
        Ok(reply)
    }
}
