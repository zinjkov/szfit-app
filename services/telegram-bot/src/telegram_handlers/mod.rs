...
pub async fn handle_message(bot: Bot, ud: UserDialogue, msg: Message) -> TelegramResult<()> {
...
pub async fn handle_callback(bot: Bot, ud: UserDialogue, cq: CallbackQuery, cat: Catalog) -> TelegramResult<()> {
    ...
    let result = CallbackHandlerProcessor::new(bot, cq, ud, cat)
        ...
        .inspect_err(|err|
            log::error!("callback raised error: {err:?}"));
    if result.is_err() {
        let _ = send_text(&cbot, chat_id, format!("something went wrong {}",
                                               result.as_ref().unwrap_err().to_string()).to_string()).await;
    }
    Ok(result?)
}


pub async fn handle_command(bot: Bot, command: TelegramCommand, msg: Message, ud: UserDialogue, cat: Catalog)
                            -> TelegramResult<()> {
...
pub async fn set_state(user_dialog: &UserDialogue, screen: Screen) -> HandlerResult<()> {
    log::info!("Set screen {:?} for user {:?}", screen, user_dialog.chat_id().0);
