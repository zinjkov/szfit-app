...
use crate::repositories::sets_repo::sets_repository::{ISetsRepository, SetsForCreate};
use crate::store;
use dill::component;
...
impl ISetsRepository for PostgresqlSetsRepository {
    async fn create(&self, sets_for_insert: Vec<SetsForCreate>) -> RepoResult<()> {
        let mut builder = QueryBuilder::new(
            "INSERT INTO sets(weight_kg, reps, exercise_id, user_id, training_id)"
        );
        builder.push_values(sets_for_insert,
                            |mut b, sfi| {
