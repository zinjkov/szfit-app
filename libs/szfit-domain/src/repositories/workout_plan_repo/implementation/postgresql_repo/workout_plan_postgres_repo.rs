...
impl IWorkoutPlanRepository for PostgresqlWorkoutPlanRepo {
    ...
    async fn find_by_id(&self, workout_id: Id) -> RepoResult<WorkoutPlan> {
        ...
        for row in res {
            let plan = plan_opt.get_or_insert_with(|| {
                WorkoutPlan {
            ...
            plan.exercise_list.push(
                Exercise::new(Id(row.exercise_id), row.exercise_name)
            );
        };
    ...
    async fn add_exercises(&self, workout_id: Id, exercise_ids: Vec<Id>) -> RepoResult<WorkoutPlan> {
        ...
        builder.push_values(exercise_ids,
                            |mut b, id| {
                                b.push_bind(*workout_id)
                                    .push_bind(*id);
                            });
