...
#[sqlx::test(
    migrator = "MIGRATOR",
    fixtures(path = "fixtures", scripts("workout"))
)]
...
#[sqlx::test(
    migrator = "MIGRATOR",
    fixtures(path = "fixtures", scripts("for_delete"))
)]
async fn test_delete(db: PgPool) {
    let repo = prepare_test(db);
    repo.delete(Id(1)).await.expect("delete failed");
}

#[sqlx::test(
    migrator = "MIGRATOR",
    fixtures(path = "fixtures", scripts("for_create"))
)]
async fn test_create(db: PgPool) {
    let repo = prepare_test(db);
    let res =
        repo.create(Id(1), "test 1".into()).await.expect("create failed");
    assert_eq!(*res.id, 1);
...
#[sqlx::test(
    migrator = "MIGRATOR",
    fixtures(path = "fixtures", scripts("test_find_by_id"))
)]
async fn test_find_by_id(db: PgPool) {
    let res =
        prepare_test(db).find_by_id(Id(1)).await.expect("find by failed");
...
#[sqlx::test(
    migrator = "MIGRATOR",
    fixtures(path = "fixtures", scripts("add_exercise"))
)]
async fn test_add_exercise(db: PgPool) {
    let repo = PostgresqlWorkoutPlanRepo::new(db);
    let plan = repo
        .add_exercises(Id(1), vec![Id(1)])
        .await
        .expect("workout list not fetched");
