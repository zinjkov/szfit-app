...
async fn test_list(db: PgPool) {
    ...
    let first = repo.list(1, 0).await.expect("first fetch failed");
    assert_eq!(first.len(), 1);
    assert_eq!(*first[0].id, 1);
    assert_eq!(first[0].name, "exercise 1");

    let second = repo.list(1, 1).await.expect("second fetch failed");
...
async fn test_find_by_id(db: PgPool) {
    ...
    let first = repo.find_by_id(Id(1)).await.expect("first fetch failed");
...
async fn test_create(db: PgPool) {
    ...
    let exercise =
        repo.find_by_id(Id(1)).await.expect("first fetch failed");
