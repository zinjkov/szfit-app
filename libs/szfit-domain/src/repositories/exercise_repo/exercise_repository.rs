...
pub trait IExerciseRepository: Send + Sync {
    ...
    async fn list(&self, limit: usize, offset: usize) -> RepoResult<Vec<Exercise>>;
    async fn list_default(&self) -> RepoResult<Vec<Exercise>> {
        self.list(self.default_limit(), 0).await
    }

    async fn list_with_limit(&self, limit: usize) -> RepoResult<Vec<Exercise>> {
        self.list(limit, 0).await
    }

    async fn create(&self, exercise_for_insert: ExerciseForCreate) -> RepoResult<Exercise>;
