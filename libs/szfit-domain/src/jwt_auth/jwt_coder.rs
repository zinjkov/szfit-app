...
impl<Decoding, Validation>
    JwtCoder<EncodingKey, Decoding, Header, Validation>
{
    pub(crate) fn encode<Claims: Serialize>(
        &self,
        claims: &Claims,
    ) -> JwtAuthResult<String> {
...
impl<Encoding, Header>
    JwtCoder<Encoding, DecodingKey, Header, Validation>
{
    pub(crate) fn decode<Claims: DeserializeOwned>(
        &self,
        token: &str,
    ) -> JwtAuthResult<Claims> {
        let token_data =
...
mod test {
    ...
    fn test_sep() {
        ...
        println!("default_claims {default_claims:?}");
        let token =
            encoder.encode(&default_claims).expect("expect token!");
        println!("token {token:?}");
        let result_claims: Claims<AuthClaims> =
            decoder.decode(token.as_str()).unwrap();
        println!("result_claims {result_claims:?}");
    }
}
